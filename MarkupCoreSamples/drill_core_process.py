...
class DrillCoreProcess:
    ...
    def process_image(self, image_name='', file_name='', metrics=None, max_side=0, resize_to=512, purpose='train', debug=False):
        ...
        image = cv2.imread(location, cv2.IMREAD_GRAYSCALE)[700:]
        ...
        cv2.imwrite(f'../ProcessorData/{dir}/images/{purpose}/resized_{file_name}', resized_image)
        # if debug:
    ...
    def fill_labeling_file(self, file_name, img, cores, resize_to, purpose):
        ...
        for core in cores:
            ...
            yolo_string.append(f'0 {x_center_norm:.6f} {y_center_norm:.6f} {width_norm:.6f} {height_norm:.6f}')

        dir = 'test_output' if self.debug else 'output'
        label_file_path = os.path.join(f'../ProcessorData/{dir}/labels/{purpose}/', f'resized_{os.path.splitext(file_name)[0]}.txt')
        ...
        try:
            with open(label_file_path, 'r') as f:
                ...
                if content:
                    print(f"File {label_file_path} already contains data. Skipping overwrite.")
                    return
        except FileNotFoundError:
            pass

        # Write to file only if it's empty or doesn't exist
